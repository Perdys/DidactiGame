...
import com.mygdx.DidacticGame.Pantallas.Juego_QQSM;
import com.mygdx.DidacticGame.Pantallas.Juego_Rosco;
...
public class Menu_Juegos extends Pantalla {
    ...
    Table table;
    CheckBox.CheckBoxStyle estilo_checkbox;
    ...
    public Menu_Juegos(DidacticGame juego) {
        this.juego = juego;
    ...
    public void render(float delta) {	/*ejecutarse todas las veces posible por segundo, ejecuten todas las acciones del juego
        ...
        batch.draw((Texture)tarea.asset.get("data/texturas/botones_juegos.png"), anchura_juego - proporcion_x(0.5) - proporcion_y(0.1), proporcion_y(0.1), proporcion_x(0.5), proporcion_y(0.25));
        batch.end();
    ...
    public void texturas_cargar() {
        estilo_checkbox = checkbox_estilo();
        ...
        for (int i = 0; i < 26; ++i){
            texturas_rosco.letras[i] = tarea.asset.get("data/texturas/juego_rosco/digitos/letras/l" + i + ".png");
            texturas_rosco.fallos[i] = tarea.asset.get("data/texturas/juego_rosco/rojo/r" + i + ".png");
            texturas_rosco.aciertos[i] = tarea.asset.get("data/texturas/juego_rosco/verde/v" + i + ".png");
            if (i < 3)
                for (int j = 0; j < 10; ++j) {
                    if (i < 2)
                        texturas_rosco.puntuacion[i][j] = tarea.asset.get("data/texturas/juego_rosco/digitos/puntuacion/p" + i + "_" + j + ".png");
                    texturas_rosco.tiempo[i][j] = tarea.asset.get("data/texturas/juego_rosco/digitos/tiempo/t" + i + "_" + j +".png");
                }
        }
        ...
        for (int i = 0; i < 4; ++i) {
            texturas_qqsm.fallos[i] = tarea.asset.get("data/texturas/juego_qqsm/rojo/r" + i + ".png");
            texturas_qqsm.aciertos[i] = tarea.asset.get("data/texturas/juego_qqsm/verde/v" + i + ".png");
        }
    }
}
