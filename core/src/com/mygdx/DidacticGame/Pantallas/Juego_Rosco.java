...
import static com.mygdx.DidacticGame.Pantallas.Menus.Menu_Juegos.*;
...
public class Juego_Rosco extends Pantalla {
    ...
    public Juego_Rosco(DidacticGame juego) {
        this.juego = juego;
    ...
    public void render(float delta) {	/*ejecutarse todas las veces posible por segundo, ejecuten todas las acciones del juego
        ...
        batch.draw((Texture)tarea.asset.get("data/texturas/fondo/rosco.png"), 0, 0, anchura_juego, altura_juego);
        if (jugadores.jugador_actual().jugando)
            batch.draw((Texture)tarea.asset.get("data/texturas/juego_rosco/boton_on.png"), 0, 0, anchura_juego, altura_juego);
        texturas_mostrar();
        batch.end();
        ...
        click = new InputAdapter() {
            public boolean touchUp (int x, int y, int pointer, int button) {
                if (boton_acierto.contains(x, y)) {
                    if (!jugadores.vacio())
                        if (jugadores.jugador_actual().jugando) {
                            jugadores.jugador_actual().letras_visitadas[jugadores.jugador_actual().letra_actual] = 1; //letra actual visitada
                            ++jugadores.jugador_actual().n_aciertos;

                            if ((jugadores.jugador_actual().n_aciertos + jugadores.jugador_actual().n_fallos) < 26) //no se ha contestado a t odo
                                letra_siguiente();
                ...
                if (boton_fallo.contains(x, y)) {
                    if (!jugadores.vacio())
                        if (jugadores.jugador_actual().jugando) {
                            ...
                            ++jugadores.jugador_actual().n_fallos;
    ...
    public void texturas_mostrar() {
        ...
        if (!jugadores.vacio()) {
            ...
            if (!jugadores.vacio()) {
                ...
                batch.draw(texturas_rosco.puntuacion[0][jugadores.jugador_actual().n_aciertos % 10], 0, 0, anchura_juego, altura_juego);
                batch.draw(texturas_rosco.puntuacion[1][jugadores.jugador_actual().n_aciertos / 10], 0, 0, anchura_juego, altura_juego);
